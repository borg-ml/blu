...
| `LIB` | `experimental` | `experimental` | `unsupported` | Initial base/string/table/math/coroutine functions and host `require` exist. Lua-family owned profiles expose host-backed `package.loaded` and `package.preload` tables; `require` invokes and caches preload functions before falling back to the host loader, while Blu/Luau retain their sandboxed host `require` without a guest `package` table. Stateful `string.gmatch`, resumable `string.gsub`, legacy `table.foreach`/`foreachi`, and profile-available `__pairs` handlers execute in the owned path with bounded pattern/operation state. Exact profile libraries, system libraries, and full return/error conventions do not. |
