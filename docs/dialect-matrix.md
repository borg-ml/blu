...
|---|---|---|---|---|
| `LEX` | `experimental` | `experimental` | `experimental` | Public legacy execution still compiles source with pinned `luau0-src` release 728. The separate bounded `blu-syntax` frontend retains raw-byte spans and trivia, reconciles byte-zero dialect directives, and gates the initial token slice by profile. Its parser, resolver, and compiler cover `local name = expression`, expression-list `return`, nil/boolean/decimal-integer/identifier expressions, escape-free quoted byte strings, grouping parentheses, unary `not`, `+`, and profile-gated `//`. Profile-specific string escapes remain explicitly rejected. |
...
| `OP` | `experimental` | `experimental` | `experimental` | Arithmetic, logical, comparison, length, concat, and Luau `//` compatibility-bytecode paths exist. The owned path directly executes `+` for all profiles and `//` for Luau and Lua 5.3–5.5. Lua 5.3 bitwise syntax/semantics and the broader owned operator set do not. |
