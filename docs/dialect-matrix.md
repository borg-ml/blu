...
| **Overall** | **`experimental`** | **`experimental`** | **`unsupported`** | The current conformance harness differentially checks a focused Luau corpus. Lua binaries currently serve only a shared portable smoke matrix. |

Lua 5.1-5.5 profiles are rejected at VM execution with a structured
not-implemented error. A parser accepting some portable Lua syntax through the
Luau bootstrap does not change those profiles to `experimental`.
