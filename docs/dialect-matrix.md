...
| `OP` | `experimental` | `experimental` | `experimental` | Arithmetic, logical, comparison, length, concat, and Luau `//` compatibility-bytecode paths exist. The owned path directly executes arithmetic and right-associative `..` for all profiles and `//` for Luau and Lua 5.3–5.5. Canonical concat uses independent operands, so bootstrap translation rejects it rather than assuming Luau-contiguous registers. Lua 5.3 bitwise syntax/semantics and the broader owned operator set do not. |
