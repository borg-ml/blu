...
| `OP` | `experimental` | `experimental` | `experimental` | Arithmetic, logical, comparison, length, concat, and Luau `//` compatibility-bytecode paths exist. The owned path directly executes arithmetic, right-associative `..`, scalar equality/ordering, and operand-returning short-circuit `and`/`or` for all profiles, plus `//` for Luau and Lua 5.3–5.5. Canonical concat and comparison instructions use independent operands; logical operators use validated forward branches. Bootstrap translation rejects these rather than assuming Luau-specific operand or control-flow shapes. Backward branches, Lua 5.3 bitwise syntax/semantics, and the broader owned operator set do not. |
