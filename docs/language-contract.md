...
One- or two-digit right-aligned field widths are supported for the implemented
conversions. One- or two-digit explicit precisions are supported for `%s`,
`%f`, `%e`, and `%E`; `%.s` selects zero precision as in the reference
grammars.
...
enforces the hard string limit. Flags, field widths wider than two digits,
integer precisions, precisions wider than two digits, `%g`/`%G`, `%a`/`%A`,
`%q`, and non-scalar `%s` behavior remain structured unsupported features
