...
#[test]
fn length_is_a_profile_neutral_token() {
    for profile in SemanticProfile::ALL {
        ...
        assert_eq!(
            significant_kinds(&lexed),
            [TokenKind::Return, TokenKind::Hash, TokenKind::StringLiteral,],
            "{profile}"
        );
    }
}
...
#[test]
fn comparison_tokens_are_shared_across_profiles() {
    for profile in SemanticProfile::ALL {
        let source = source(b"return a == b, a ~= b, a < b, a <= b, a > b, a >= b".to_vec());
        let lexed = lex(&source, profile, LexerLimits::default()).unwrap();
        assert!(!lexed.has_errors(), "{profile}");
        let comparisons: Vec<_> = significant_kinds(&lexed)
            .into_iter()
            .filter(|kind| {
                matches!(
                    kind,
...
#[test]
fn while_loop_keywords_are_shared_across_profiles() {
    for profile in SemanticProfile::ALL {
        let source = source(b"while ready do ready = false end".to_vec());
        let lexed = lex(&source, profile, LexerLimits::default()).unwrap();
        assert!(!lexed.has_errors(), "{profile}");
        assert_eq!(
            significant_kinds(&lexed)
                .into_iter()
                .filter(|kind| matches!(kind, TokenKind::While | TokenKind::Do | TokenKind::End))
                .collect::<Vec<_>>(),
            [TokenKind::While, TokenKind::Do, TokenKind::End],
            "{profile}"
        );
    }
}
...
#[test]
fn for_loop_keywords_are_shared_across_profiles() {
    for profile in SemanticProfile::ALL {
        let source = source(b"for index in iterator do end".to_vec());
        let lexed = lex(&source, profile, LexerLimits::default()).unwrap();
        assert!(!lexed.has_errors(), "{profile}");
        assert_eq!(
            significant_kinds(&lexed)
                ...
                .filter(|kind| {
                    matches!(
                        kind,
                        TokenKind::For | TokenKind::In | TokenKind::Do | TokenKind::End
                    )
                })
                .collect::<Vec<_>>(),
            [TokenKind::For, TokenKind::In, TokenKind::Do, TokenKind::End]
